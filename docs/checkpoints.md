...
6. **Build memory (R5.6):** `tool/capped_build.sh` wraps every Flutter build with a hard `MEM_MAX` (8G) ceiling at `nice -n 10`, plus `.NOTPARALLEL:` and `make doctor`. Verified end to end: `make all` produced `dist/dayforge-1.0.0.apk` + `dist/dayforge_1.0.0_amd64.deb` with the machine never dropping below 5.5 GB available (build scope peaked ~4.3 GB inside its 8 GB ceiling).
...
- **The memory ceiling has to be a cgroup, not a flag.** `org.gradle.jvmargs=-Xmx3G` (set in session 13) bounds only the Gradle JVM; the AOT `gen_snapshot` processes are separate and unbounded, which is how a 14 GB machine with no swap still lost its desktop session.
- **…and a snap escapes the cgroup you put it in.** The first cut of that fix was a plain `systemd-run --user --scope -p MemoryMax=8G -- flutter build`, and it did nothing: Flutter is a **classic snap**, and `snap run` leaves the caller's cgroup for a fresh `snap.flutter.flutter-<uuid>.scope` under `app.slice`. Reading the running Gradle process's own cgroup (`cut -d: -f3 /proc/$PID/cgroup`, then `cat /sys/fs/cgroup$CG/memory.max`) is what exposed it — it printed `max`. **Always verify a containment fix from inside the contained process**, never from the command line you wrote. `tool/capped_build.sh` now also applies the ceiling to the new snap scope via `systemctl --user set-property --runtime`.
