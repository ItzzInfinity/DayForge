...
- [x] R5.6 `make all` / `make apk` no longer risk taking the desktop down — done 2026-08-19; builds run under `systemd-run --user --scope -p MemoryMax=8G -p MemorySwapMax=0` at `nice -n 10`, `.NOTPARALLEL:` stops `-j` running the Android and Linux builds together, and `make doctor` reports the ceiling, the containment status and this machine's RAM/swap. Overshoot now OOM-kills inside the build's cgroup only. Escape hatches: `MEM_MAX=`, `ABI=android-arm64`, `CAP=` to opt out
